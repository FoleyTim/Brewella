...
import * as visemes from "../assets/mockVisemes.json";
...
export function Avatar(props) {
  ...
  const { message, onMessagePlayed, chat, text } = useChat();
  const [lipsync, setLipsync] = useState();
  const [mockLipsync, setMockLipsync] = useState();
  ...
  useFrame(() => {
    ...
    if (mockLipsync) {
      ...
      for (let i = 0; i < visemes.default.length; i++) {
        const mouthCue = visemes.default[i];
        if (
          currentAudioTime >= mouthCue.start &&
          currentAudioTime <= mouthCue.end
        ) {
